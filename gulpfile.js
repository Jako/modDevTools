...
gulp.task('scripts-mgr', function () {
    return gulp.src([
        ...
        'source/js/mgr/widgets/properties.grid.js',
        'source/js/mgr/widgets/settings.panel.js',
        'source/js/mgr/sections/home.js'
    ])
        .pipe(concat('moddevtools.min.js'))
        .pipe(uglify())
        .pipe(header(banner + '\n', {pkg: pkg}))
        .pipe(gulp.dest('assets/components/moddevtools/js/mgr/'))
});

gulp.task('sass-mgr', function () {
    return gulp.src([
        'source/sass/mgr/moddevtools.scss'
    ])
        ...
        .pipe(footer('\n' + banner, {pkg: pkg}))
        .pipe(gulp.dest('assets/components/moddevtools/css/mgr/'))
});

gulp.task('sass-breadcrumbs', function () {
    return gulp.src([
        'source/sass/mgr/breadcrumbs.scss'
    ])
        .pipe(sass().on('error', sass.logError))
        .pipe(postcss([
            autoprefixer()
...
gulp.task('bump-copyright', function () {
    return gulp.src([
        'core/components/moddevtools/model/moddevtools/moddevtools.class.php',
        'core/components/moddevtools/src/ModDevTools.php',
    ], {base: './'})
